...
1. 读取并校验配置。
2. 创建 `OneBotClient`、`CodeBuddyClient`、`SessionManager`、`DashboardService`、`PluginManager`。
3. 创建 `MessageHandler`，把 Dashboard、屏蔽服务、插件管理器注入进去。
4. 注册内置插件：
   - `SelfLearningPlugin`
   - `VoiceBroadcastPlugin`
5. 加载外部插件并收集 Dashboard 扩展路由。
6. 连接 OneBot。
7. 启动调度器、Dashboard 和通知监听。
...
- 统一管理内置插件和 `PLUGIN_PATHS` 指向的外部插件
- 提供消息钩子、Prompt 注入、命令处理和 Dashboard 路由注册
...
- 运行时真正生效的默认值，以 `src/types/config.ts` 为准，不以示例 `.env` 为准。
